...
export const ensureValidToken = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      ...
      // ✅ Throttle: Only check token every 30 seconds
      const now = Date.now();
      const thirtySeconds = 30 * 1000;
      
      if (now - auth.lastTokenCheck < thirtySeconds && auth.user) {
        // Return cached validation result
        return !!getAccessToken();
      }
      
      // Set validating flag
      thunkAPI.dispatch(setTokenValidating(true));
  ...
  async (_, thunkAPI) => {
    try {
      ...
      if (auth.profileLoading || auth.isProfileFetching) {
        return thunkAPI.rejectWithValue("Profile fetch already in progress");
      }
...
// ✅ Get Profile Silently (with proper throttling)
export const getProfileSilently = createAsyncThunk(
  "api/auth/getProfileSilently",
  async (_, thunkAPI) => {
    try {
