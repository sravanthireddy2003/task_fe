...
// 2FA Component
const TwoFactorAuth = () => {
  const user = useSelector(selectUser); // Add user selector
  ...
  const check2FAStatus = async () => {
    try {
      const response = await httpGetService('api/auth/2fa/status');
      if (response?.success) {
  ...
  const handleEnable2FA = async () => {
    ...
    try {
      ...
      if (response?.success) {
        // Backend provides the QR code image
        if (response.qr) {
          ...
          toast.error('QR code not generated by server');
        }
      } else {
  ...
  const handleVerify2FA = async () => {
    ...
    setLoading(true);
    try {
      const response = await httpPostService('api/auth/2fa/verify', {
        token: verificationCode  // Backend expects 'token' field
      });
