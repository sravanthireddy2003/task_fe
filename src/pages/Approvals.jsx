...
const Approvals = () => {
  ...
  // Handle approve reassignment request
  const handleApproveRequest = async (requestId, requestData) => {
    ...
    try {
      ...
      toast.success(resp?.message || 'Reassignment request approved!');
      
      // Update the request status
      setRawRequests(prev => prev.map(req => 
        req.id === requestId 
          ? { ...req, status: 'APPROVED', requestData: { ...req.requestData, status: 'APPROVED' } }
          : req
      ));
