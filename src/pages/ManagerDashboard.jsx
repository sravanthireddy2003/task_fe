...
// const readNumberFromStorage = (keys, fallback) => {
...
// const readStringFromStorage = (keys, fallback) => {
//   if (typeof window === 'undefined') return fallback;
//   for (const key of keys) {
//     const raw = window.localStorage.getItem(key);
//     if (raw) return raw;
...
// const dedupeTasks = (...lists) => {
//   const map = new Map();
//   lists.flat().forEach((task) => {
//     const key = task?.id || task?._id || task?.task_id || task?.public_id;
//     if (!key) return;
//     map.set(key, task);
//   });
...
// const deriveProjectCount = (tasks) => {
//   const set = new Set();
//   tasks.forEach((task) => {
//     const pid = task?.project_id ?? task?.projectId ?? task?.project?.id ?? task?.project?.public_id;
//     if (pid) set.add(pid);
//   });
//   return set.size;
// };

// const deriveClientCount = (tasks) => {
//   const set = new Set();
//   tasks.forEach((task) => {
//     const cid = task?.client_id ?? task?.clientId ?? task?.client?.id;
//     if (cid) set.add(cid);
//   });
//   return set.size;
// };
