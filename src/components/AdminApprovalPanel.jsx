...
// import { Check, Eye, Loader2, RefreshCw, Lock, X, Clock, User, FileText, CheckCircle, AlertCircle, Calendar, BarChart3, Paperclip } from 'lucide-react';
...
//         // Check if project is now closed
//         if (result.data && result.data.newStatus === 'CLOSED') {
//           setClosedProjects(prev => new Set([...prev, requestId]));
//         }

//         // Show success message from response
//         toast.success(result.message || 'Project closure approved successfully');

//         // Update the request status locally instead of refreshing
//         // This provides immediate UI feedback
//         // Note: In a real app, you might want to refresh to get updated data from server
//         // dispatch(fetchPendingApprovals('ADMIN'));
//       } else {
...
//               <div key={request.id} className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
//                 {/* Header Section */}
...
//                         }`}>
//                           {closedProjects.has(request.id) ? 'CLOSED' : approvedRequests.has(request.id) ? 'APPROVED' : request.status}
//                         </span>
//                       </div>
...
const AdminApprovalPanel = () => {
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        // Check if project is now closed based on response
        if (result.data && result.data.newStatus === 'CLOSED') {
          setClosedProjects(prev => new Set([...prev, requestId]));
        }
  ...
  const openRejectModal = (request) => {
    // Don't open reject modal if project is closed
  ...
  // Helper function to check if project is closed
  const isProjectClosed = (request) => {
    // Check if in closedProjects Set
    if (closedProjects.has(request.id)) {
      return true;
    }
    // Check if request status indicates it's closed
    if (request.status === 'CLOSED' || request.to_state === 'CLOSED') {
      return true;
    }
    return false;
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      {/* Reject Modal */}
      {showRejectModal && selectedRequest && !isProjectClosed(selectedRequest) && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Reject Project Closure</h3>
